/**
 * Google Apps Script yang diperbaiki untuk menangani struktur spreadsheet yang ada
 */
    ...
    if (action === "getPosts") {
      ...
      for (var j = 1; j < dataPostingan.length; j++) {
        ...
        var postData = {
          ...
          username: row[7] || "User", // Add username column if missing
          imageUrl: row[8] || "", // Add image URL support
        }
    ...
    if (dataUsers[i][1] && dataUsers[i][1].toLowerCase() === params.email.toLowerCase()) {
      return ContentService.createTextOutput(
        JSON.stringify({
          ...
          role: dataUsers[i][7] || "user", // Make sure role is from column H (index 7)
...
function handleCreatePost(params, sheetPosts) {
  ...
  // Columns: ID Users, ID Postingan, timestamp, Judul, Deskripsi, Like, Dislike, Username, ImageUrl
