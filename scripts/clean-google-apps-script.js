 ...
 * Versi bersih tanpa deklarasi variabel bermasalah
 */
...
function doGet(e) {
  try {
    ...
    if (action === "getPosts") {
      ...
      console.log("Raw posting data: " + JSON.stringify(dataPostingan))
      ...
      console.log("Processed posts: " + JSON.stringify(postsArray))
    ...
    console.log("Error in doGet: " + e.toString())
...
function doPost(e) {
  try {
    ...
    console.log("Error in doPost: " + e.toString())
    return ContentService.createTextOutput(
      JSON.stringify({
        error: "Server error: " + e.message,
        details: e.toString(),
...
function handleLogin(params, sheetUsers) {
  try {
    ...
    console.log("Looking for user with email: " + params.email)

    for (var i = 1; i < dataUsers.length; i++) {
      if (dataUsers[i][1] && dataUsers[i][1].toLowerCase() === params.email.toLowerCase()) {
        console.log("User found: " + JSON.stringify(dataUsers[i]))
...
function handleRegistration(params, sheetUsers) {
  try {
    ...
    console.log("User registered successfully: " + idUsers)
...
function handleCreatePost(params, sheetPosts) {
  try {
    ...
    console.log("Create post error: " + e.toString())
    return ContentService.createTextOutput(
      JSON.stringify({
...
function handleLikeDislike(params, sheetPosts, sheetLikes) {
  try {
    ...
    console.log("Like/Dislike error: " + e.toString())
    return ContentService.createTextOutput(
      JSON.stringify({
  ...
  try {
    ...
    for (var i = 1; i < dataUsers.length; i++) {
      if (dataUsers[i][0] === params.idUsers) {
        userRole = dataUsers[i][7] || "user" // Role is in column H (index 7)
        break
      }
    }
    ...
    console.log("Delete post error: " + e.toString())
    return ContentService.createTextOutput(
      JSON.stringify({
...
function handleUpdateProfile(params, sheetUsers) {
  try {
    ...
    console.log("Update profile error: " + e.toString())
    return ContentService.createTextOutput(
      JSON.stringify({
