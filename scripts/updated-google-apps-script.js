/**
 * Google Apps Script untuk Mahasiswa Feedback Platform dengan fitur update profile
 */
  ...
  try {
    ...
    for (var i = 1; i < dataUsers.length; i++) {
      if (dataUsers[i][0] === params.idUsers) {
        // Update user data
        // Columns: idUsers, email, username, password, nim, gender, jurusan, role, timestamp, bio, location, website
        sheetUsers.getRange(i + 1, 3).setValue(params.username || dataUsers[i][2]) // username
        sheetUsers.getRange(i + 1, 2).setValue(params.email || dataUsers[i][1]) // email
        sheetUsers.getRange(i + 1, 5).setValue(params.nim || dataUsers[i][4]) // nim
        sheetUsers.getRange(i + 1, 7).setValue(params.jurusan || dataUsers[i][6]) // jurusan

        // Add new columns for bio, location, website if they don't exist
        if (dataUsers[i].length < 12) {
          sheetUsers.getRange(i + 1, 10).setValue(params.bio || "") // bio
          sheetUsers.getRange(i + 1, 11).setValue(params.location || "") // location
          sheetUsers.getRange(i + 1, 12).setValue(params.website || "") // website
        } else {
          sheetUsers.getRange(i + 1, 10).setValue(params.bio || dataUsers[i][9]) // bio
          sheetUsers.getRange(i + 1, 11).setValue(params.location || dataUsers[i][10]) // location
          sheetUsers.getRange(i + 1, 12).setValue(params.website || dataUsers[i][11]) // website
        }
